...
export function calculateOrbitalElements(
    { velocity, position }: StateVectors,
    mass: number
): OrbitalElements {
    const specificAngularMomentum = new Vector3().crossVectors(
        position,
        velocity
    );
    ...
    const positionUnitVector = new Vector3().copy(position).normalize();
    ...
    const semiLutusRectum =
        specificAngularMomentum.lengthSq() / standardGravitationalParameter;
    ...
    {
        const n = new Vector3(
            specificAngularMomentum.z,
            specificAngularMomentum.x,
            0
        );
    ...
    const trueAnomaly = Math.acos(
        eccentricityVector.dot(position) / (eccentricity * position.length())
    );
...
export function calculateStateVectors(
    ...
    const velocityLength = Math.sqrt(
        standardGravitationalParameter * (2 / length - 1 / semiMajorAxis)
    );

    const pane = new Euler(inclination, longitudeOfAscendingNode, 0, 'XYZ');

    const position = new Vector3(
        Math.cos(argumentOfPeriapsis),
        0,
        -Math.sin(argumentOfPeriapsis)
    ).multiplyScalar(length);
    const velocity = new Vector3(
        -Math.sin(argumentOfPeriapsis),
        0,
        -Math.cos(argumentOfPeriapsis)
    ).multiplyScalar(velocityLength);
